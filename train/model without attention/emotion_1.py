...
set_seed(2711)
writer = SummaryWriter(comment = 'lr0.0001boredommeanvaluehubertlarge')
...
class CNNLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes):
        ...
        self.fc_emotion_1 = nn.Sequential(
    ...
    def forward(self,x, a):
        ...
        lstm_out = unpacked
        fc_emotion_1_out = torch.zeros((x.shape[0], lstm_out.shape[2])).to(device)
        for i in range(x.shape[0]):
            fc_emotion_1_out[i,:] = torch.mean(lstm_out[i,0:a[i],:],dim=0)
        fc_emotion_1_out = self.fc_emotion_1(fc_emotion_1_out)
        return fc_emotion_1_out
...
def train(epoch):
    ...
    for i, (signals, emotions, a) in enumerate(train_loader):
        ...
        emotion_1 = emotions[:,1].to(device).reshape(-1,1)
        lstm_output_1 = cnnlstm_model(signals, a)
        loss_1 = criterion(lstm_output_1, emotion_1)
        avg_loss = (avg_loss * i + loss_1.item())/(i+1)
        optimizer.zero_grad()
        loss_1.backward()
        optimizer.step()
        if (i+1) % 1 == 0:
            print (f'Epoch [{epoch+1}], Step [{i+1}/{train_steps}], avg_Loss: {avg_loss:.4f}, Loss = {loss_1:.4f}')
    writer.add_scalar("0.0001_loss_1_boredom_meanvalue_hubert_large/train", avg_loss, epoch)
...
def evaluate(epoch):
    ...
    with torch.no_grad():
        ...
        for i, (signals, emotions, b) in enumerate(devel_loader):
            signals = signals.to(device)
            emotion_1 = emotions[:,1].to(device).reshape(-1,1)
            lstm_output_1 = cnnlstm_model(signals, b)
            size = len(emotion_1)
            act[count_valid:count_valid+size,:] = emotion_1
            pre[count_valid:count_valid+size,:] = lstm_output_1
            count_valid += size
            if (i+1) % 1 == 0:
        ...
        correlation_1 = stats.spearmanr(act, pre)[0]
        writer.add_scalar("0.0001_eval_correlation_1_boredom_meanvalue_hubert_large/devel", correlation_1, epoch)

def save_checkpoint(state, filename='/data/boredomhubertlarge/checkpoint.pth.tar'):
...
for epoch in range(iters):
    ...
    }, filename=f'/data/boredomhubertlarge/epoch_{epoch+1}.pth.tar')
    evaluate(epoch)