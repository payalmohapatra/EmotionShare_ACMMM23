...
set_seed(2711)
writer = SummaryWriter(comment = 'lr0.0001excitementmeanvaluehubertlarge')
...
class CNNLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes):
        ...
        self.fc_emotion_5 = nn.Sequential(
    ...
    def forward(self,x, a):
        ...
        lstm_out = unpacked
        fc_emotion_5_out = torch.zeros((x.shape[0], lstm_out.shape[2])).to(device)
        for i in range(x.shape[0]):
            fc_emotion_5_out[i,:] = torch.mean(lstm_out[i,0:a[i],:],dim=0)
        fc_emotion_5_out = self.fc_emotion_5(fc_emotion_5_out)
        return fc_emotion_5_out
...
def train(epoch):
    ...
    for i, (signals, emotions, a) in enumerate(train_loader):
        ...
        emotion_5 = emotions[:,5].to(device).reshape(-1,1)
        lstm_output_5 = cnnlstm_model(signals, a)
        loss_5 = criterion(lstm_output_5, emotion_5)
        avg_loss = (avg_loss * i + loss_5.item())/(i+1)
        optimizer.zero_grad()
        loss_5.backward()
        optimizer.step()
        if (i+1) % 1 == 0:
            print (f'Epoch [{epoch+1}], Step [{i+1}/{train_steps}], avg_Loss: {avg_loss:.4f}, Loss = {loss_5:.4f}')
    writer.add_scalar("0.0001_loss_5_excitement_meanvalue_v2_hubert_large_v2/train", avg_loss, epoch)
...
def evaluate(epoch):
    ...
    with torch.no_grad():
        ...
        for i, (signals, emotions, b) in enumerate(devel_loader):
            signals = signals.to(device)
            emotion_5 = emotions[:,5].to(device).reshape(-1,1)
            lstm_output_5 = cnnlstm_model(signals, b)
            size = len(emotion_5)
            act[count_valid:count_valid+size,:] = emotion_5
            pre[count_valid:count_valid+size,:] = lstm_output_5
            count_valid += size
            if (i+1) % 1 == 0:
        ...
        correlation_5 = stats.spearmanr(act, pre)[0]
        writer.add_scalar("0.0001_eval_correlation_5_excitement_meanvalue_v2_hubert_large_v2/devel", correlation_5, epoch)

def save_checkpoint(state, filename='/data/excitementhubertlarge/checkpoint.pth.tar'):
...
for epoch in range(iters):
    ...
    }, filename=f'/data/excitementhubertlarge/epoch_{epoch+1}.pth.tar')
    evaluate(epoch)