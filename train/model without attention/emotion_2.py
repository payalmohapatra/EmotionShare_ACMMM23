...
set_seed(2711)
writer = SummaryWriter(comment = 'lr0.0001calmnessmeanvaluehubertlarge')
...
class CNNLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes):
        ...
        self.fc_emotion_2 = nn.Sequential(
    ...
    def forward(self,x, a):
        ...
        lstm_out = unpacked
        fc_emotion_2_out = torch.zeros((x.shape[0], lstm_out.shape[2])).to(device)
        for i in range(x.shape[0]):
            fc_emotion_2_out[i,:] = torch.mean(lstm_out[i,0:a[i],:],dim=0)
        fc_emotion_2_out = self.fc_emotion_2(fc_emotion_2_out)
        return fc_emotion_2_out
...
def train(epoch):
    ...
    for i, (signals, emotions, a) in enumerate(train_loader):
        ...
        emotion_2 = emotions[:,2].to(device).reshape(-1,1)
        lstm_output_2 = cnnlstm_model(signals, a)
        loss_2 = criterion(lstm_output_2, emotion_2)
        avg_loss = (avg_loss * i + loss_2.item())/(i+1)
        optimizer.zero_grad()
        loss_2.backward()
        optimizer.step()
        if (i+1) % 1 == 0:
            print (f'Epoch [{epoch+1}], Step [{i+1}/{train_steps}], avg_Loss: {avg_loss:.4f}, Loss = {loss_2:.4f}')
    writer.add_scalar("0.0001_loss_2_calmness_meanvalue_v2_hubert_large_v2/train", avg_loss, epoch)
...
def evaluate(epoch):
    ...
    with torch.no_grad():
        ...
        for i, (signals, emotions, b) in enumerate(devel_loader):
            signals = signals.to(device)
            emotion_2 = emotions[:,2].to(device).reshape(-1,1)
            lstm_output_2 = cnnlstm_model(signals, b)
            size = len(emotion_2)
            act[count_valid:count_valid+size,:] = emotion_2
            pre[count_valid:count_valid+size,:] = lstm_output_2
            count_valid += size
            if (i+1) % 1 == 0:
        ...
        correlation_2 = stats.spearmanr(act, pre)[0]
        writer.add_scalar("0.0001_eval_correlation_2_calmness_meanvalue_v2_hubert_large_v2/devel", correlation_2, epoch)

def save_checkpoint(state, filename='/data/calmnesshubertlarge/checkpoint.pth.tar'):
...
for epoch in range(iters):
    ...
    }, filename=f'/data/calmnesshubertlarge/epoch_{epoch+1}.pth.tar')
    evaluate(epoch)